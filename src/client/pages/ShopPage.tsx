    ...
    async function loadData() {
      ...
      try {
        ...
        if (sortParam === "price-asc") {
          fetchedProducts.sort((a: Product, b: Product) => (a.pricePerKg ?? 0) - (b.pricePerKg ?? 0));
        } else if (sortParam === "price-desc") {
          fetchedProducts.sort((a: Product, b: Product) => (b.pricePerKg ?? 0) - (a.pricePerKg ?? 0));
        ...
        <ScrollAnimation className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-stone-100 pb-4">
            <div>
              ...
              <p className="text-[13px] text-stone-500 font-medium">Home / Shop / Fresh Meats</p>
            </div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-50 border border-red-200 text-primary text-xs font-bold w-fit">
              <span>★ 100% Fresh Cutting Daily</span>
            </div>
          </div>
        </ScrollAnimation>
        ...
        <ScrollAnimation delay={0.2}>
          {loading ? (
            ...
            <div className="text-center py-20 bg-stone-50 rounded-2xl border border-stone-200 p-8">
              <h3 className="text-xl font-bold text-stone-900 mb-2">No meat cuts available</h3>
              <p className="text-stone-500 max-w-md mx-auto text-sm">
                No meat cuts match the selected criteria. Items added by the store administrator will appear here.
              </p>
            </div>
          )}
        </ScrollAnimation>
